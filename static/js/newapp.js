function LoadImage_NavBar(id, image) {
    var x = document.createElement("IMG");
    x.setAttribute("class", "profile_image");
    x.setAttribute("src", image);
    x.setAttribute("height", "30px");
    x.setAttribute("width", "30px");
    x.setAttribute("title", "profile image");
    var child = document.getElementById("nav_name_" + id);
    document.getElementById("user_image_" + id).appendChild(x);
};
...
function LoadProfileImage(id, photo) {
    window.addEventListener("load", function () {
        var x = document.createElement("IMG");
        x.setAttribute("class", "rounded-circle img-responsive");
        x.setAttribute("style", "margin-right: 5px;");
        ...
        x.setAttribute("title", "profile image");
        document.getElementById("profile-img-" + id).appendChild(x);
    });
}
...
function ShowPassword() {
    x = document.getElementById('password_login');
    y = document.getElementById('show-pass')
    if (x.type == 'password') {
        x.type = 'text';
    } else {
        x.type = 'password';
...
function ShowPasswordReset() {
    x = document.getElementById('password_reset');
    y = document.getElementById('show-pass-reset')
    if (x.type == 'password') {
        x.type = 'text';
    } else {
        x.type = 'password';
...
async function registerSW() {
    if ('serviceWorker' in navigator) {
        try {
            await navigator.serviceWorker.register('https://newapp.nl/sw.js');
        } catch (e) {
...
function Save_Post(id) {
    fetch('https://newapp.nl/save-post/' + id)
        .then(response => response.json())
        .then(data => {
            if (data.operation == 'saved') {
...
function Follow_Tag(tag) {
    fetch('https://newapp.nl/follow-tag/' + tag)
        .then(response => response.json())
        .then(data => {
            if (data.operation == 'followed') {
                x = document.getElementById('follow-tag-' + tag);
                x.innerHTML = '&#x2713 Following';
            } else if (data.operation == 'unfollowed') {
                x = document.getElementById('follow-tag-' + tag);
                x.innerHTML = 'Follow';
            }
        })
...
function Like_Post(id) {
    fetch('https://newapp.nl/like-post/' + id)
        .then(response => response.json())
        .then(data => {
...
function Follow_User(id){
    fetch('https://newapp.nl/follow-user/' + id)
        .then(response => response.json())
        .then(data => {
            x = document.getElementById('follow-user-' + id);
            if (data.operation == 'unfollowed') {
                x.innerHTML = 'Follow';
            } else if (data.operation == 'followed') {
                x.innerHTML = '&#x2713 Following';
            }
...
function init_NewApp() {
    /*Notification.requestPermission().then(function(result) {
        if(result === 'granted') {
            
        }
    });*/
   // var styles = LoadAfter.loadFiles();
